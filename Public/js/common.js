...
$(function(){
		...
		$.ajax({
			...
			success:function(result){
				...
				if(result.status===true){
					...
					<img src="'+'/group'+data[i].goods_img+'" />\
				</a>\
				<div>\
					<h4>\
	 ...
	 * 点击清空最近浏览
	 */
	$('body').on('click','#clearRecentView',function(){
		...
		$.ajax({
			...
			success:function(result){
				...
				if(result.status === true){
					...
					for(var i in data){
						var nodeStr = '<li>\
							<a class="image" href="/group/Home/Detail/index/gid/'+data[i].gid+'">\
								<img src="'+'/group'+data[i].goods_img+'" />\
							</a>\
							<div>\
								<h4>\
									<a  href="/group/Home/Detail/index/gid/'+data[i].gid+'">'+data[i].main_title+'</a>\
								</h4>\
					...
					$('#my-cart .menu').html('<li>没有添加到购物的商品!</li>');
				}
			}
		})
